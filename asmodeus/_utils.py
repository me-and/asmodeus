...
JSONVal: TypeAlias = Union[None, bool, str, int, float, list['JSONVal'],
                           dict[str, 'JSONVal']]
...
# TODO Could this be changed to actually allow base string/int/float values?
...
class JSONableDate(datetime.datetime, JSONable):
    ...
    def __new__(cls, year_or_str_or_dt: Union[int, str, datetime.datetime],
                ...
                microsecond: int = 0, tzinfo: Optional[datetime.tzinfo] = None,
                *, fold: int = 0) -> Self:
        if isinstance(year_or_str_or_dt, int):
            # Looks like a year, so emulate the normal datetime.datetime
            # interface.
        ...
        if isinstance(year_or_str_or_dt, str):
            # It's a string.  If it's in ISO format, injest it directly,
            # otherwise see what TaskWarrior's calc function makes of it.
            try:
                dt = datetime.datetime.fromisoformat(year_or_str_or_dt)
            except ValueError:
                # TODO This shouldn't be duplicating code in
                # TaskWarrior.calc_datetime, but I wanted a quick solution
...
class JSONableDuration(datetime.timedelta, JSONable):
    ...
    def __new__(cls,
                ...
                minutes: int = 0, hours: int = 0, weeks: int = 0) -> Self:
...
# TODO This has __setitem__ handling whether an item is JSONable already; it
# should also have similar function for if it's called with something that's
# not JSONable...
class JSONableDict(dict[str, Jb], JSONable, Generic[Jb], ABC):
    _key_class_map: Mapping[str, type[Jb]]  # Would like this to be a ClassVar, but PEP526 says that's not (yet) supported
    _required_keys: ClassVar[Iterable[str]] = ()
    _fallback_class: Optional[type[Jb]] = None  # Would like this to be a ClassVar, but PEP526 says that's not (yet) supported
    ...
    @overload
    def __init__(self, __map: SupportsKeysAndGetItem[str, object]) -> None: ...
    @overload
    def __init__(self, __map: SupportsKeysAndGetItem[str, object], **kwargs: object) -> None: ...
    @overload
    def __init__(self, __it: Iterable[tuple[str, object]]) -> None: ...
    @overload
    def __init__(self, __it: Iterable[tuple[str, object]], **kwargs: object) -> None: ...
    def __init__(self, *args: Union[SupportsKeysAndGetItem[str, object],
        ...
        if len(args) == 0:
            ...
            try:
                ...
                arg = cast(SupportsKeysAndGetItem[str, object], arg)
                processed_arg = ((key, self._parse_if_needed(key, arg[key]))
                                 for key in keys)
            super().__init__(processed_arg, **processed_kwargs)
        else:
            raise TypeError(
                f'{self.__class__.__name__} accepts at most 1 positional argument')
...
class JSONableList(list[Jb], JSONable, Generic[Jb], ABC):
    _class: type[Jb]  # Would like this to be a ClassVar, but PEP526 says that's not (yet) supported
...
class JSONableUUIDList(JSONableList[JSONableUUID]):
    _class = JSONableUUID  # Would like this to be a ClassVar, but PEP526 says that's not (yet) supported


class JSONableStringList(JSONableList[JSONableString]):
    _class = JSONableString  # Would like this to be a ClassVar, but PEP526 says that's not (yet) supported
...
def _json_pre_dump(obj: object) -> JSONValImmut:
    if isinstance(obj, JSONable):
        return obj._json_pre_dump()
    raise TypeError(f'Cannot convert {obj} to JSONVal')
...
def load_json(string: str) -> JSONVal:
    # Safe to cast as json.loads can only return these items given we know it's
    # using its default parser.
    return cast(JSONVal, json.loads(string))


def dump_json(val: JSONable | JSONValImmut) -> str:
    return json.dumps(val, default=_json_pre_dump)
